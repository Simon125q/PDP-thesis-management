...
func (m *Model) AllRealizedThesisEntries(sort_by string, desc_order bool, page_num, page_limit int, queryParams url.Values) ([]types.RealizedThesisEntry, error) {
	query := fmt.Sprintf(`
        ...
        COALESCE(ct.chair_academic_title, '') AS chair_title,
        ...
        COALESCE(ct.reviewer_academic_title, '') AS reviewer_title, 
...
func (m *Model) GetTheListOfAllTitlesNames() ([]string, error) {
	query := `
        SELECT DISTINCT COALESCE(current_academic_title, '')
        FROM University_Employee
    `
